...
class MainViewController: UIViewController, UsersListViewControllerDelegate, UITableViewDelegate, UITableViewDataSource, TaskInfoViewControllerDelegate {
    ...
    var filterdata = DataSource.allTasks
    var refreshControl = UIRefreshControl()

    @IBAction func didChangeSegment(_ sender: UISegmentedControl?) {
        filterdata = []
        if segmentedControl.selectedSegmentIndex == 0{
            filterdata = DataSource.allTasks
        }
    ...
    func initialize() {
        ...
        //---------------------------------------------
        let myCustomCell = UINib(nibName: TaskTableViewCell.identifier, bundle: nil)
        
        tableView.register(myCustomCell, forCellReuseIdentifier: TaskTableViewCell.identifier)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let task = DataSource.allTasks[indexPath.row]
        if task.done {
            cell.textLabel!.text = "✔️ \(task.description)"
        } else {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return DataSource.allTasks.count
    }
